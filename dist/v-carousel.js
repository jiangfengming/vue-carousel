...
var script = {
  ...
  methods: {
    ...
    onPanMove: function onPanMove(e) {
      var panPos = this.isHorizontal ? e.detail.clientX : e.detail.clientY;
      ...
      if (panPos !== this.prevPanPos) {
        ...
        this.panPos = panPos;
        var offset = this.offset + (this.isHorizontal ? e.detail.offsetX : e.detail.offsetY);
    ...
    onPanEnd: function onPanEnd(e) {
      ...
      if (this.panOffset !== this.offset || this.transitionOffsets.indexOf(this.offset) === -1) {
        var offset;
        var v0 = Math.abs(this.panPos - this.prevPanPos) / Math.abs(e.timeStamp - this.prevPanTime);
        ...
        if (v0 >= a * Math.sqrt(2 * Math.abs(this.nextOffset - this.panOffset) / a)) {
          ...
          if (Math.abs(this.panOffset - this.offset) < this.threshold || this.panOffset < this.offset && nearOffset < this.offset || this.panOffset > this.offset && nearOffset > this.offset) {
            offset = nearOffset;
          } else {
            offset = nextOffset;
          }
